...
hist_breaks <- seq(from = , to =, length.out = num_breaks)
```
...
x_factor_2 <- cut(x_dbl, breaks = ) ### set the breaks and add the new argument
...
iris_group %>% 
  ggplot(mapping = aes(x = )) +
  geom_linerange(mapping = aes(ymin = ,
                               ymax = )) +
  geom_point(mapping = aes(y = ),
             size = )
```
