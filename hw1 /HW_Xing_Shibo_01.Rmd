...
```{r, solution_01d}
iris %>% ggplot(mapping = aes(x = Sepal.Length)) + 
  geom_histogram(bins = 6, size = 1.55, color = 'navyblue', fill = 'gold')
```
...
```{r, solution_01e}
iris %>% ggplot(mapping = aes(x = Sepal.Length)) + 
  geom_histogram(bins = 6, size = 1.55, color = 'navyblue', fill = 'gold', alpha = 0.5)
```
...
my_iris <- iris %>% 
  mutate(sepal_length_bin = cut(iris$Sepal.Length, breaks = hist_breaks, include.lowest = TRUE))

my_iris$sepal_length_bin
...
```{r, solution_03a}
select(my_iris, 'sepal_length_bin') %>% summary()
```
...
my_iris %>% 
  ...
  #n() function returns the number observations of the current group
...
Here we map the sepal_length_bin to the x-axis. 'sepal_length_bin' was created by cut() function with 7 breakpoints, creating 6 unique values. 
Thus it is discrete data. After counting the observations of each unique value, we set the x-axis of the value label.

In problem 1, we map the Sepal.length to the x-axis, which is continuous data.
Thus, we set the bin count to an arbitrary number, dividing the continuous data into 6 chunks.

Thus, the x-axes of the histograms look differently in these two problems. 
...
my_iris %>% 
  group_by(sepal_length_bin) #grouping doesn't change how data looks
```
...
The n() function gets the same result as the count() function. The difference between the two is simply how we format them. n() can only be used in certain dplyr functions like summarise(), which returns the number of observation in each group. If we want to achieve the same result with count(), we simply pipe the result of the group_by() to the count() function. 
...
Our histogram shows the increases and decreases of the number of observations. The change in the frequency of the data might not be clear in the native historgram. With the line segments in the histogram, we can more easily envision the distrbution of the data set. On the other hand, in the native histogram, many outliers and the uneven bars hinders our ability to make sense of the distribution. 
...
The trend shown in 5b mainly shows us how color affects the price of a diamond. Yet according to the box plot in 5b, color is not a dominant factor of price. All 7 color have similar price ranges. 
5f on the other hand, shows us that the real determinant factor of price is carat, specifically as in IF cut. As it clearly shows that diamonds that have less carats are significant cheaper than those with more carats.
...
$$ 
\frac{\partial{f}}{\partial{x_{1}}} = abx_{1}^{b - 1} 
$$
...
$$ 
\frac{\partial{f}}{\partial{x_{2}}} = cdx_{2}^{d - 1}
$$
...
$$ 
\frac{\partial^{2}{f}}{\partial{x_{1}^{2}}} = (ab^{2} - ab)x_{1}^{b - 2}
$$
...
$$ 
  x^{T} x = (1 \times N) \cdot (N \times 1) = (1 \times 1) 
$$
...
$$
  inner\ product: x^T * x = (1 \times\ N) \cdot (N \times 1) = (1 \times 1) \\
  outer\ product: x * x^{T} = (N \times\ 1)\cdot (1 \times N) = (N \times N)
$$
