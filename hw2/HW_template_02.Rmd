...
$$ 
\begin{aligned}
Binomial(H|\mu,AB) \propto log(\mu^{H}(1 - \mu)^{AB - H})  \\
= log(\mu^{H}) + log(1 - \mu)^{AB - H}  \\ 
= H\cdot log(\mu) + (AB - H)\cdot log(1 - \mu) \\ 
= 108\cdot log(\mu) + 412\cdot log(1-\mu)
\end{aligned}
$$
...
It corresponds to the peak of the plotted curve, where the derivative of the curve with respect with $\mu$ is 0.
...
tibble::tibble(
  ...
  mutate(log_lik = log_binom_pmf_unnorm(events =player_hits, 
                                        trials = player_atbats, 
                                        prob = mu)) %>%
  ggplot(mapping = aes(x = mu, y = log_lik)) + 
  geom_line() + 
...
tibble::tibble(
  ...
  mutate(log_lik = log_binom_pmf(events =player_hits, 
                                        trials = player_atbats, 
                                        prob = mu)) %>%
  ggplot(mapping = aes(x = mu, y = log_lik)) + 
  geom_line() + 
...
Yes it is the same location as before.
...
tibble::tibble(
  ...
  mutate(log_lik = dbinom(x = player_hits, 
                          size = player_atbats, 
                          prob = mu, 
                          log = TRUE)) %>%
  ggplot(mapping = aes(x = mu, y = log_lik)) + 
  geom_line() + 
  geom_vline(mapping = aes(), xintercept = 0.3139535)
```
...
length(coin_probs)
```
it corresponds to observing exactly 0 heads out of 11 trails. 
...
sum(coin_probs)
```
The probability of observing less than or equal to 5 heads out of 11 flips.
...
$$ 
\begin{aligned}
Beta(\mu \mid a,b) \propto \log(\mu^{a-1}(1-\mu)^{b-1}) \\ 
...
$$
\begin{aligned}
p(\mu|m,N) \propto Binomial(m|N,\mu) \cdot Beta(\mu|a,b) \\
p(\mu|m,N) =  \mu^m(1-\mu)^{N-m}\mu^{a-1}(1-\mu)^{b-1} \\
p(\mu|m,N) \propto \mu^{m+a-1}(1-\mu)^{N-m+b-1} \\
p(\mu|m,N) = Beta(\mu,a+m,b+(N-m)) \\ 
a_{new} = a + m, b_{new} = b + (N - m)
\end{aligned}
$$
...
0.05 quantile tells us that only 5 percent of probability is less than 0.21, meaning that the 0.2 probability MLE based on the first 30 bats might be highly unrepresentative of the player. 

MLE based on 602 is greater than 0.05 quantile probability and slightly less than that of the 0.95 quantile. The mode of the probability is in between of 0.05 and 0.95 quantile, which is also the range of MLE probability based on a sufficient number of observations. 
...
b_post_start <- start_atbats + b_prior - start_hits

a_post_player <- player_hits + a_prior
...
Both MLEs are higher than their posterior means since player has higher probability of hits this season. The prior data indicates a worse performance the player had before. Since MLEs are solely based on the observations of this season, the posterior takes into account of both prior and observations, the posteriors are lower than MLEs. 
...
```
MLE based on 602 observations is within the middle 90% uncertainty intervals, the MLE based on 30 observations is not within this range.
...
pbeta(q = 1, shape1 = a_prior, shape2 = b_prior) - pbeta(q = 0.3, shape1 = a_prior, shape2 = b_prior) 

pbeta(q = 1, shape1 = a_post_player, shape2 = b_post_player) - pbeta(q = 0.3, shape1 = a_post_player, shape2 = b_post_player) 
```
I would they are likely to be a player. After the evaluation of the whole season, we found out that the integral from 0.3 quantile to 1 quantile takes up 65%, meaning we are 65% confident that the player will hit more than 30%.
...
We will continue to assume all observations are conditionally independent given the parameters. The likelihood therefore factors into the product of $N$ normal likelih oods:  
...
$$
\begin{aligned}
\log(p(\sigma  \mid  \mathbf{x}, \mu)) \propto log(\prod_{n=1}^{N}(\frac{1}{\sigma\sqrt{2\pi}}e^{-\frac{1}{2 \sigma^2}(x_n-\mu)^2}) \times \lambda \exp \left(-\lambda \sigma \right)) \\
\log(p ( \sigma  \mid  \mathbf{x}, \mu)) \propto \sum_{n=1}^{N}(log(\frac{1}{\sigma})+log(\frac{1}{\sqrt{2\pi}}) -\frac{(x_n-\mu)^2}{2\sigma^2}) + \log(\lambda) -\lambda \sigma  \\
\log(p ( \sigma \mid  \mathbf{x}, \mu)) \propto \sum_{n=1}^{N}(log(\frac{1}{\sigma}) -\frac{(x_n-\mu)^2}{2 \sigma^2 })  -\lambda \sigma \\  
\log(p ( \sigma \mid  \mathbf{x}, \mu)) \propto \sum_{n=1}^{N}(log(\frac{1}{\sigma})) -\sum_{n=1}^{N}(\frac{(x_n-\mu)^2}{2 \sigma^2})  -\lambda \sigma  \\
\log(p ( \sigma \mid \mathbf{x}, \mu)) \propto N\cdot log(\frac{1}{\sigma}) -(\frac{\sum_{n=1}^{N}((x_n-\mu)^2)}{2 \sigma^2})  -\lambda \sigma 
\end{aligned}
$$
...
#### SOLUTION
$$
log( p( \sigma  \mid \mathbf{x}, \mu)) \propto N \cdot log(\frac{1}{\sigma})-\frac{N\cdot v}{2 \sigma^2}-\lambda \sigma 
$$

As we have proven with our simplified expression, the log posterior depends on v, the statistical function rather than individual observations in x. Thus, the posterior of $\sigma$ given vector x and $\mu$ is the same as the posterior of $\sigma$ given v and $\mu$. Thus, we can say that v can be refered to as being sufficient.
...
{
  res = N * log(sigma ^ -1) - (N * v) / (2 * sigma ^ 2) - lambda * sigma
  return(res)
}
```
...
It has only one mode. The shape looks lagorithmic.
...
This may seem like an intimidating expression, but it simply sazys plug in the expression for $\sigma$ in terms of $\varphi$ (the inverse of the link function) into the log-posterior expression on $\sigma$. Then add in the log of the derivative of the inverse link function with respect to $\varphi$.  
...
$$
\sigma = e^\varphi \\
\frac{d}{d\varphi}(g^{-1}(\varphi)) = \frac{de^\varphi}{d\varphi} = e^\varphi \\
\log(\frac{d}{d\varphi}(g^{-1}(\varphi)) = \varphi
$$
...
log_post_varphi_unnorm <- function(varphi, N, v, lambda)
{
  ...
  log_post <- N * log(sigma ^ -1) - (N * v) / (2 * sigma ^ 2) - lambda * sigma
  
  # account for the derivative adjustment
  log_post <- log_post + varphi
...
Yes it looks more like a parabola now.