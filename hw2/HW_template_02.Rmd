...
sum(coin_probs)
```
The probability is the sum of the coin_probs vector.
...
$$
\begin{aligned}
p(\mu|m,N) \propto Binomial(m|N,\mu)Beta(\mu|a,b) \\
p(\mu|m,N) = Beta(\mu|a,b) \propto \mu^m(1-\mu)^{N-m}\mu^{a-1}(1-\mu)^{b-1} \\
p(\mu|m,N) \propto \mu^{m+a-1}(1-\mu)^{N-m+b-1} \\
Posterior: p(\mu|m,N) = Beta(\mu,a+m,b+(N-m))
\end{aligned}
$$
...
b_post_start <- start_atbats + b_prior

a_post_player <- player_hits + a_prior
b_post_player <- player_atbats + b_prior
```
...
they are the same as their corresponding MLEs. MLE gives us the most probable mean, which is when the derivative of Beta distribution is 0, of which the expression equals to # of events / # of trials.
...
```
both MLEs based on 30 and 602 observations are both contained within the middle 90% uncertainty intervals in their corresponding posterior distributions.
...
```
No I would not. The probability that the player will have a hit probability greater than 0.3 is only 8.79e-0.5, which is very slim. Although they have a good prior record, the observations of this season really bring them down. 
...
It has only one mode. The shape does look like a parabola.
