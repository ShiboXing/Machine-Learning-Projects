---
title: "CS 1675 Spring 2020: Homework 06"
...
train_data <- readr::read_csv("https://raw.githubusercontent.com/jyurko/CS_1675_Spring_2020/master/hw_data/hw06/hw_06_train_data.csv", col_names = TRUE)
...
```{r, solution_01a_a}
train_data %>% ggplot(mapping = aes(x = x, y = y)) +
  geom_point() 
...
```{r, solution_01e}
### design 
Xmat <- model.matrix(~ x, train_data)
info_two_units <- list(
  yobs = train_data$y,
  design_matrix = Xmat,
  num_hidden = 2,
...
info_two_units$num_params <- 2 * (1 + 1) + 2 + 1 #H(D + 1) + H + 1
```
...
my_neuralnet_sse <- function(theta, my_info)
{
  ...
  f <- H %*% aw + a0
  # calculate the SSE
  t(my_info$yobs - f) %*% (my_info$yobs - f)
}
```
...
```{r, solution_01f_b}
my_neuralnet_sse(c(0, 0, 0, 0, 0, 0, 0), info_two_units)
```

Test out your `my_neuralnet_sse()` function with values of -1.25 for all parameters.  

```{r, solution_01f_c}
my_neuralnet_sse(c(-1.25, -1.25, -1.25, -1.25, -1.25, -1.25, -1.25), info_two_units)
```
...
```{r, solution_01g_b}
optim_fit_2_b <-  optim(rep(-1.25, 7) ,
                       my_neuralnet_sse,
                       gr =  NULL,
                       info_two_units,
                       method = 'BFGS',
...
optim_fit_2_b$par
```
Although the optimizers in both cases have their gradients converge to zero, they result in distinct parameters. This could be because that they converged to different local minima of the SSE values along the function curve. 
...
$$
\frac{\partial \delta_n}{\partial \alpha_k} =2 \cdot (y - \alpha_0 - h_{n,k}\cdot\alpha_k) \cdot h_{n,k} = -2 \cdot \delta_{n} \cdot h_{n,k}
$$
...
$$ 
\sum_{n=1}^N\frac{\partial \delta_n}{\alpha_k} = -2 \cdot \sum_{n=1}^N\delta_{n} \cdot h_{n,k} \\
 = -2 \cdot \delta^T \cdot H_{:,k} 
$$
...
**Using your result from Problem 2b), write the vector of "complete" derivatives using $\mathbf{H}$ and $\boldsymbol{\delta}$. What is the dimensionality of this vector? Are you missing an output layer parameter? If so, which one?**  

#### SOLUTION

$$
  -2   \cdot \delta ^T \cdot H \ \ (1 \times K)
$$
Yeah, we are missing one parameter, $a_0$.
...
$$
\frac{\partial}{\partial \alpha_0} \left(SSE\right) = \frac{\partial(y_n - \alpha_0 - \sum_{k=1}^{K}(h_{n,k}\cdot\alpha_k)) ^ 2}{\partial\alpha_0}
$$

$$
\frac{\partial}{\partial \alpha_0} \left(SSE\right) = -2 \cdot \delta  
$$

$$
  \frac{\partial}{\partial g_\alpha} (SSE) = -2 \delta^T \cdot [1\ \ H ]  \ (append \ a\ column\ of\ 1s)
$$
...
$$
g_{n,k}^{\prime} = h_{n,k}(1 - h_{n,k}) \\ 
G_p = \sum_n^N\sum_k^K(h_{n,k}(1 - h_{n,k})) \\
G_p = H  \circ  (1 - H)
$$
...
backprop_grad_batch <- function(theta, my_info)
{
  ...
  E <- AW * Gp * rep(d, my_info$num_hidden)
...
```{r, solution_03c_a}
backprop_grad_batch(rep(0, 7), info_two_units)
```
Since all 7 parameters, including the hidden parameters beta and fully-connected layers' alphas are all zeros, the changes on hidden parameters values won't have any effect on the output response unless the alphas are changes. The output response, currently zero, can only be changed if alphas are non-zeros Thus, the derivative of the hidden layer parameters are zeros as well. 
...
```{r, solution_03d_a}
optim_backprop_2_a <- optim(rep(0, 7) ,
                       my_neuralnet_sse,
                       gr =  NULL,
                       info_two_units,
                       method = 'BFGS',
...
solution:
the optimal parameter estimates are the same as the optim estimations based on initial guess of all zeros. It seems like the finite gradient fitting and back-propagation to the beta parameters all fall into the same minima.
...
**Create the list of required information to fit a neural network with 11 hidden units. After creating the list, you must calculate the total number of parameters in the model. How many parameters are there compared to the model with just 2 hidden units?**  
...
info_11_units$num_params <- info_11_units$num_hidden * ncol(Xmat) + info_11_units$num_hidden + 1
```
...
No we don't need to. Since the Beta matrix still has the same # of rows as before, which are the unknown parameters in the original function. 
...
there are 50 observations in each fold's test set.
the one with 5 hidden units is the best.
...
```{r, solution_04c_a}
nnet_grid <- expand.grid(layer1 = seq(2, 14, 3),
                         layer2 = seq(0, 6, 2),
                         layer3 = 0)
```
...
the best model is the one where layer has 14 units layer 2 and 3 have zero (where RMSE is the smallest)
...
#### SOLUTION
The model of which layer 1 has 5 would and layer 2 has 0 unit would be the good enough. It has the lowest RMSE values. 
