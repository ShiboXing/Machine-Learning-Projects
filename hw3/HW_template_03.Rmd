...
$$
p(\mu, \sigma \mid \mathbf{x}) \propto \prod^N_{n=1}(normal(x_n \mid \mu,g^{-1} \left( \varphi \right))) \cdot normal(\mu \mid \mu_0, \tau_0) \cdot uniform(p(g^{-1} \left( \varphi \right) \mid l,u)) \cdot |\frac{d g^{-1} \left( \varphi \right)}{d\phi}|
$$
...
values 5 to 10 for $\varphi$ are completely ruled out, -0.5 seems to be the most probable value for $\mu$
