...
df_01 <- readr::read_csv("https://raw.githubusercontent.com/jyurko/CS_1675_Spring_2020/master/hw_data/hw03/df_01.csv", col_names = TRUE)
...
df_01 %>% ggplot(mapping = aes(x = obs_id, y = x)) +
  ...
  geom_point()
```
Most values appear to be gaussian distributed, there are only 1 or 2 outliers.
### 1b)
...
most of the values concentrate at the x = 0, the shape looks like a symmetric bell-curve, which is a normal distrbution. 
...
df_01$x %>% quantile(probs = c(0.05, 0.95))
### calculate the empirical standard deviation of x
df_01$x %>% sd()
```

Yes, the median is very close to 0.5, as indicated by both graphs. The histogram's display is also in-line with the Quartile information shown in the summary.
...
```{r, 1d, eval=TRUE}
df_01 %>% mutate(rolling_avg = cummean(df_01$x)) %>% 
  ggplot(mapping = aes(x = obs_id, y = rolling_avg)) +
  geom_line() 

```
The 0 to the 100 appears the most variable, it become steady after about 100.
...
Since Laplace Approximation is strictly symmetric, in its MVN the sigma values ranges from negative infinity to positive infinity. Yet our log-posterior surface disallows sigmas that are negative. Thus, we need to find some method to allow unbounded sigmas in our log-possterior surface. 
...
$$
p(\mu, \sigma \mid \mathbf{x}) \propto \prod^N_{n=1}(normal(x_n \mid \mu,g^{-1} \left( \varphi \right))) \cdot p(\mu \mid \mu_0, \tau_0) \cdot p(g^{-1}(\varphi)) \cdot |\frac{d g^{-1} \left( \varphi \right)}{d\phi}|
$$
...
my_logpost <- function(unknowns, my_info)
{
  ...
  log_deriv_adjust <- lik_varphi # sigma = e^phi -> d(sigma)/d(phi) = e^phi
...
### bounds on mu
mu_lwr <- qnorm(0.01, info_inform_N05$mu_0, info_inform_N05$tau_0)
mu_upr <- qnorm(0.99, info_inform_N05$mu_0, info_inform_N05$tau_0)


### bounds on varphi
varphi_lwr <- log(qexp(0.01, info_inform_N05$sigma_rate))
varphi_upr <- log(qexp(0.99, info_inform_N05$sigma_rate))

### create the grid 
param_grid <- expand.grid(mu = seq(mu_lwr, mu_upr, length.out = 201),
                          varphi = seq(varphi_lwr, varphi_upr, length.out = 201),
                          KEEP.OUT.ATTRS = FALSE,
                          stringsAsFactors = FALSE) %>% 
  as.data.frame() %>% tbl_df()
...
```
 ...
 $\varphi$ less than about 0.5 is completely ruled out, -0.5 seems to be the most probable value for $\mu$
...
As the sample size increases, the log-posterior surface becomes more gaussian looking around the posterior mode. Yes both parameters shift as the sample size increases. The uncertainty also decreases contour-line area shrinks. 
...
#### SOLUTION
both optimization results, including the parameters, and the hessian matrices are largely the same. The difference between them is number of calls fn and gr. Two different init guesses require two different numbers of iterations. 

Since our log-posterior surface is unimodal, it is reasonable to expect two optim calls to reach a singular mode value. Since our initial guesses start from different areas in the posterior graph. It is only natural for them to reach the mode with different iterations of approximation.
...
post_samples_inform_N45 %>% summary()
post_samples_inform_N405 %>% summary()
...
```
the standard deviation of both the mu and sigma shrinks as the observation count increases.
...
**What is the posterior robability that $\mu$ is positive for each of the three observation sample sizes? What are the 0.05 and 0.95 quantiles on $\sigma$ for each of the three observation sample sizes?**  

#### SOLUTION

```{r, solution_03g, eval=TRUE}
#the probability of mu > 0
sum(post_samples_inform_N05$mu > 0) / nrow(post_samples_inform_N05)
sum(post_samples_inform_N45$mu > 0) / nrow(post_samples_inform_N45)
sum(post_samples_inform_N405$mu > 0) / nrow(post_samples_inform_N405)

#0.05 and 0.95 quantiles of sigma
post_samples_inform_N05$sigma %>% quantile(c(0.05, 0.95))
post_samples_inform_N45$sigma %>% quantile(c(0.05, 0.95))
post_samples_inform_N405$sigma %>% quantile(c(0.05, 0.95))
...
df_02_high %>% ggplot(mapping = aes(x = x, y = y)) +
  geom_point()

df_02_low %>% ggplot(mapping = aes(x = x, y = y)) +
  geom_point()
```
the functional form looks like a quadratic function
...
mod_low_linear <- lm(y ~ I(x) , df_02_low)
mod_low_linear 

mod_low_quad <- lm(y ~ I(x^2) + I(x), df_02_low)
mod_low_quad

mod_low_8th <- lm(y ~ I(x^8) + I(x^7) + I(x^6) + I(x^5) +
                    I(x^4) + I(x^3) + I(x^2) + I(x), df_02_low)
```

The high noise case models are given in the code chunk below.  

```{r, solution_04b_b, eval=TRUE}
mod_high_linear <- lm(y ~ I(x), df_02_high)

mod_high_quad <- lm(y ~ I(x^2) + I(x), df_02_high)

mod_high_8th <- lm(y ~ I(x^8) + I(x^7) + I(x^6) + I(x^5) +
                    I(x^4) + I(x^3) + I(x^2) + I(x), df_02_high)
```
...
mod_low_quad %>% summary()
mod_low_8th %>% summary()

mod_high_linear %>% summary()
mod_high_quad %>% summary()
mod_high_8th %>% summary()
```
For the low-noise data, the 8th-order model has the highest R squared.
For the high-noise case, the 8th-order model also has the highest R squared.
...
f_quad <- y ~ I(x ^ 2) + I(x)
mod_low_quad_fold_01 <- lm(f_quad, low_noise_train_fold_01)

mod_low_quad_fold_02 <- lm(f_quad, low_noise_train_fold_02)

mod_low_quad_fold_03 <- lm(f_quad, low_noise_train_fold_03)

mod_low_quad_fold_04 <- lm(f_quad, low_noise_train_fold_04)

mod_low_quad_fold_05 <- lm(f_quad, low_noise_train_fold_05)
```

The 8th order polynomial fits should be specified below.  

```{r, solution_04f_b, eval=TRUE}
f_8th <- y ~ I(x^8) + I(x^7) + I(x^6) + I(x^5) + I(x^4) + I(x^3) +
  I(x^2) + I(x)

mod_low_8th_fold_01 <- lm(f_8th, low_noise_train_fold_01)

mod_low_8th_fold_02 <- lm(f_8th, low_noise_train_fold_02)

mod_low_8th_fold_03 <- lm(f_8th, low_noise_train_fold_03)

mod_low_8th_fold_04 <- lm(f_8th, low_noise_train_fold_04)

mod_low_8th_fold_05 <- lm(f_8th, low_noise_train_fold_05)
```
...
#### SOLUTION
```{r, solution_4g}
cv_low_quad_fold_train_rmse <- c(modelr::rmse(mod_low_quad_fold_01, low_noise_train_fold_01),
                                modelr::rmse(mod_low_quad_fold_02, low_noise_train_fold_02),
                                modelr::rmse(mod_low_quad_fold_03, low_noise_train_fold_03),
                                modelr::rmse(mod_low_quad_fold_04, low_noise_train_fold_04),
                                modelr::rmse(mod_low_quad_fold_05, low_noise_train_fold_05))

cv_low_8th_fold_train_rmse <- c(modelr::rmse(mod_low_8th_fold_01, low_noise_train_fold_01),
                                modelr::rmse(mod_low_8th_fold_02, low_noise_train_fold_02),
                                modelr::rmse(mod_low_8th_fold_03, low_noise_train_fold_03),
                                modelr::rmse(mod_low_8th_fold_04, low_noise_train_fold_04),
                                modelr::rmse(mod_low_8th_fold_05, low_noise_train_fold_05))

mean(cv_low_quad_fold_train_rmse)
mean(cv_low_8th_fold_train_rmse)
```
the 8th order model, which is the more complex model, has the lower average RMSE
...
#### SOLUTION
```{r, solution_4h}
cv_low_quad_fold_test_rmse <- c(modelr::rmse(mod_low_quad_fold_01, low_noise_test_fold_01),
                                modelr::rmse(mod_low_quad_fold_02, low_noise_test_fold_02),
                                modelr::rmse(mod_low_quad_fold_03, low_noise_test_fold_03),
                                modelr::rmse(mod_low_quad_fold_04, low_noise_test_fold_04),
                                modelr::rmse(mod_low_quad_fold_05, low_noise_test_fold_05))

cv_low_8th_fold_test_rmse <- c(modelr::rmse(mod_low_8th_fold_01, low_noise_test_fold_01),
                                modelr::rmse(mod_low_8th_fold_02, low_noise_test_fold_02),
                                modelr::rmse(mod_low_8th_fold_03, low_noise_test_fold_03),
                                modelr::rmse(mod_low_8th_fold_04, low_noise_test_fold_04),
                                modelr::rmse(mod_low_8th_fold_05, low_noise_test_fold_05))
...
```
the quadratic relationship has the lower avergae cross-validation RMSE.  