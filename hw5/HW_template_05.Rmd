...
```{r, solution_01a}
Xmat = model.matrix(y ~ I(x1) + I(x2) + I(x1 * x2), prob_01_df)
```
4 columns, the 4th column corresponds to the interaction term
...
SSmat[1,1]
```
The intercept column, which is the first column in Xmat consists of 100 ones. Thus the [1][1] element of SSmat, the inner product of Xmat, is the inner product of the first column of Xmat, which is 100.
...
the calculation is:
$$
  \sum_{n}^{N}{Xmat_{n, 2} \cdot Xmat_{n, 3}} 
$$
...
```{r, solution_01e}
SSmat
c(which(SSmat == SSmat[2, 3]))
SSmat[c(which(SSmat == SSmat[2, 3]))]
```
The elements from the bottom left corner to the rop right corner are all equal to each other.
Since the values on the 4th column of Xmat is x1 * x2, on the 1th column are all ones, the (1,4) and (4,1) elements on sstable should be all x1 * x2. Also the elements (2,3) and (3,2) are the inner product of column 2 and 3 in Xmat, which are also x1 * x2, thus (1,4), (2,3), (3,2) and (4,1) are all the same.
...
cov2cor(bcov_5)
```
No, the correlation matrices are the same with different sigmas.
...
$$
\begin{aligned}
  f(\beta, \sigma) = \sum_n^N{(-\frac{log(\sigma^2)}{2} - \frac{log(\pi)}{2} - \frac{(y_n - \mu_n)^2}{2})} \\
  f(\beta, \sigma) \propto -\frac{N}{2} \cdot log(\sigma^2) - \frac{1}{2 \cdot \sigma^2} (y - X\beta)^t(y - X\beta)
\end{aligned}
$$
...
$$
\begin{aligned}
  f(\beta, \sigma) = log(\sum_{d=0}^{D} (\mathrm{normal}( \beta_d | \mu_{\beta}, \tau_{\beta} )))  \\
  f(\beta, \sigma) = \sum_{d=0}^{D}(-\frac{1}{2}log(\tau_\beta^2) - \frac{1}{2}log(2\pi) - \frac{1}{2\tau_\beta^2}(\beta_d - \mu_\beta)^2)\\
  f(\beta, \sigma) = \frac{1}{2\tau_\beta^2}\sum_{d=0}^{D}(\beta_d - \mu_\beta)^2
\end{aligned}
$$
log prior on beta does not depend on sigma.
...
$$
\begin{aligned}
  f(\beta, \sigma) = log(\mathrm{Exp} \left( \sigma \mid \nu \right)) \\ 
  f(\beta, \sigma) = log(\nu \cdot e^{-\nu\sigma}) \\ 
  f(\beta, \sigma) \propto  \nu \sigma 
\end{aligned}
$$
log-prior on sigma does not depend on beta
...
$$
\log\left[ p \left( \boldsymbol{\beta}, \sigma \mid \mathbf{X},\mathbf{y} \right) \right] \propto -N \cdot log(\sigma^2) - \frac{1}{2 \cdot \sigma^2} (y - X\beta)^t(y - X\beta) - \frac{1}{2\tau_\beta^2}\sum_{d=0}^{D}(\beta_d - \mu_\beta)^2 - \nu \sigma 
$$
...
$$
\log\left[ p \left( \boldsymbol{\beta}, \sigma \mid \mathbf{X},\mathbf{y} \right) \right] \propto -\frac{N}{2} \cdot log(\sigma^2) - \frac{1}{2 \cdot \sigma^2} (y - X\beta)^t(y - X\beta)  - \frac{1}{2\tau_\beta^2}\sum_{d=0}^{D}（\beta_d^2） - \nu \sigma 
$$
The beta prior then would become the equation above. The beta prior still has effects on the log posterior because only the mode of the prior distribution of beta changes. What really controls the strength of the prior should be tau_beta, which is the variable determining how diffuse the prior distribution of beta is.
...
$$
\log\left[ p \left( \boldsymbol{\beta}, \sigma \mid \mathbf{X},\mathbf{y} \right) \right] \propto -{N} \cdot log(\sigma) - \frac{1}{2 \cdot \sigma^2}\cdot SSE -  \frac{1}{2\tau_\beta^2}\sum_{d=0}^{D}（\beta_d^2） - \nu \sigma
$$
...
$$ 
\log \left[p\left(\boldsymbol{\beta},\varphi \mid \mathbf{X}, \mathbf{y}\right)\right] = \log \left[p\left(\boldsymbol{\beta},  g^{-1}(\varphi) \mid \mathbf{X}, \mathbf{y}\right)\right] + log(\frac{d(g^{-1}(\varphi))}{d\varphi})
$$
...
\begin{aligned}
\log \left[p\left(\boldsymbol{\beta},\varphi \mid \mathbf{X}, \mathbf{y}\right)\right] = \log \left[p\left(\boldsymbol{\beta},  g^{-1}(\varphi) \mid \mathbf{X}, \mathbf{y}\right)\right] + log(\frac{d(g^{-1}(\varphi))}{d\varphi}) \\
 = -{N} \cdot log(g^{-1}(\varphi)) - \frac{1}{2 \cdot g^{-1}(\varphi)^2}\cdot SSE - \frac{1}{2\tau_\beta^2}\sum_{d=0}^{D}（\beta_d^2） - \nu g^{-1}(\varphi) + \varphi \\
 = -{N} \cdot log(exp(\varphi)) - \frac{1}{2 \cdot exp(\varphi)^2}\cdot SSE - \frac{1}{2\tau_\beta^2}\sum_{d=0}^{D}（\beta_d^2） - \nu \cdot exp(\varphi) + \varphi \\
 = -{N} \cdot \varphi - \frac{1}{2 \cdot e^{2\varphi}}\cdot SSE -  \frac{1}{2\tau_\beta^2}\sum_{d=0}^{D}（\beta_d^2） - \nu \cdot e^\varphi + \varphi
\end{aligned}
$$
...
$$
\begin{aligned}
\frac{d(\log \left[p\left(\boldsymbol{\beta},\varphi \mid \mathbf{X}, \mathbf{y}\right)\right])}{d\varphi} = -N  -\frac{SSE}{2} \cdot\frac{d(exp(\varphi^{-2}))}{d\varphi} - \nu \cdot e^\varphi + 1 \\
= -N  -\frac{SSE}{2} \cdot e^{-2 \varphi} \cdot (-2) - \nu \cdot e^\varphi + 1 \\
= -N  +SSE \cdot e^{-2\varphi} - \nu \cdot e^\varphi + 1 
\end{aligned}
$$
...
\frac{d^2p(\boldsymbol{\beta},\varphi \mid \mathbf{X}, \mathbf{y})}{d\varphi^2} = -2SSE \cdot e^{-2\varphi} - \nu \cdot e^\varphi 
$$
...
$$
\begin{aligned}
\frac{d(\log \left[p\left(\boldsymbol{\beta},\varphi \mid \mathbf{X}, \mathbf{y}\right)\right])}{d\varphi} = 0 \\
-N  +SSE \cdot e^{-2\varphi} - \nu \cdot e^\varphi + 1 = 0 \\
SSE_{OLS} \cdot e^{-2\varphi} = N - 1 \\
\varphi_{MLE} = -\frac{1}{2}log(\frac{N - 1}{SSE_{OLS}}) 
...
post_samples_a <- generate_lm_post_samples(laplace_result_a, ncol(info_04$design_matrix), 2500)

post_samples_a$sigma %>% median
post_samples_a$sigma %>% quantile(0.25)
post_samples_a$sigma %>% quantile(0.75)

post_samples_a$beta_03 %>% median
post_samples_a$beta_03 %>% quantile(0.25)
post_samples_a$beta_03 %>% quantile(0.75)
```
median, 25th and 75th quantile of sigma are 0.3357911, 0.3200083 and 0.3526003 
median, 25th and 75th quantile of beta3 are -0.4432624, -0.4623174 and -0.4244938
...
info_04_weak <- list(
  yobs = y_stan,
  design_matrix = Xmat,
  mu_beta = info_04$mu_beta,
  tau_beta = 25,
  sigma_rate = info_04$sigma_rate
)

laplace_result_weak <- my_laplace(c(0, 0, 0, 0, 0), lm_logpost, info_04_weak)

post_samples_weak <- generate_lm_post_samples(laplace_result_weak, ncol(info_04$design_matrix), 2500)

post_samples_weak$sigma %>% median
post_samples_weak$sigma %>% quantile(0.25)
post_samples_weak$sigma %>% quantile(0.75)

post_samples_weak$beta_03 %>% median
post_samples_weak$beta_03 %>% quantile(0.25)
post_samples_weak$beta_03 %>% quantile(0.75)
```
the quantiles on sigma and beta are almost the same as those generated by a less diffuse beta prior.
...
post_samples_strong <- generate_lm_post_samples(laplace_result_strong, ncol(info_04$design_matrix), 2500)


post_samples_strong$sigma %>% median
post_samples_strong$sigma %>% quantile(0.25)
post_samples_strong$sigma %>% quantile(0.75)

post_samples_strong$beta_03 %>% median
post_samples_strong$beta_03 %>% quantile(0.25)
post_samples_strong$beta_03 %>% quantile(0.75)
```
The changes on the quantiles are drastically different than before because of a strong constraint on the prior distribution, all the values are much higher than before.
...
```{r, solution_05a}
X05 <- model.matrix(y ~ I(x1) + I(x2) + I(x3) + I(x4), prob_05_df)
```
...
$$
SSE = X^TSX = \sum^N_n(\mu_n (1 - \mu_n)x_{n,:}^Tx_{n,:})
$$
