...
$$
\begin{aligned}
  f(\beta, \sigma) = log(\prod_{d=0}^{D} (\mathrm{normal}( \beta_d | \mu_{\beta}, \tau_{\beta} )))  \\
  f(\beta, \sigma) = \sum_{d=0}^{D}(-\frac{1}{2}log(\tau_\beta^2) - \frac{1}{2}log(2\pi) - \frac{1}{2\tau_\beta^2}(\beta_d - \mu_\beta)^2)\\
  f(\beta, \sigma) = -\frac{D}{2}log(\tau_\beta^2) - \frac{1}{2\tau_\beta^2}\sum_{d=0}^{D}(\beta_d - \mu_\beta)^2
\end{aligned}
$$
...
$$
\log\left[ p \left( \boldsymbol{\beta}, \sigma \mid \mathbf{X},\mathbf{y} \right) \right] \propto -N \cdot log(\sigma^2) - \frac{1}{2 \cdot \sigma^2} (y - X\beta)^t(y - X\beta) -{D}\cdot log(\tau_\beta) - \frac{1}{2\tau_\beta^2}\sum_{d=0}^{D}(\beta_d - \mu_\beta)^2 - \nu \sigma 
$$
...
$$
\log\left[ p \left( \boldsymbol{\beta}, \sigma \mid \mathbf{X},\mathbf{y} \right) \right] \propto -\frac{N}{2} \cdot log(\sigma^2) - \frac{1}{2 \cdot \sigma^2} (y - X\beta)^t(y - X\beta) - {D}\cdot log(\tau_\beta) - \frac{1}{2\tau_\beta^2}\sum_{d=0}^{D}（\beta_d^2） - \nu \sigma 
$$
...
$$
\log\left[ p \left( \boldsymbol{\beta}, \sigma \mid \mathbf{X},\mathbf{y} \right) \right] \propto -{N} \cdot log(\sigma) - \frac{1}{2 \cdot \sigma^2}\cdot SSE - {D}\cdot log(\tau_\beta) - \frac{1}{2\tau_\beta^2}\sum_{d=0}^{D}（\beta_d^2） - \nu \sigma
$$
...
\begin{aligned}
\log \left[p\left(\boldsymbol{\beta},\varphi \mid \mathbf{X}, \mathbf{y}\right)\right] = \log \left[p\left(\boldsymbol{\beta},  g^{-1}(\varphi) \mid \mathbf{X}, \mathbf{y}\right)\right] + log(\frac{d(g^{-1}(\varphi))}{d\varphi}) \\
 = -{N} \cdot log(g^{-1}(\varphi)) - \frac{1}{2 \cdot g^{-1}(\varphi)^2}\cdot SSE - {D}\cdot log(\tau_\beta) - \frac{1}{2\tau_\beta^2}\sum_{d=0}^{D}（\beta_d^2） - \nu g^{-1}(\varphi) + \varphi \\
 = -{N} \cdot log(exp(\varphi)) - \frac{1}{2 \cdot exp(\varphi)^2}\cdot SSE - {D}\cdot log(\tau_\beta) - \frac{1}{2\tau_\beta^2}\sum_{d=0}^{D}（\beta_d^2） - \nu \cdot exp(\varphi) + \varphi \\
 = -{N} \cdot \varphi - \frac{1}{2 \cdot e^{2\varphi}}\cdot SSE - {D}\cdot log(\tau_\beta) - \frac{1}{2\tau_\beta^2}\sum_{d=0}^{D}（\beta_d^2） - \nu \cdot e^\varphi + \varphi
\end{aligned}
$$
...
```
The changes on the quantiles are slightly different than before but still extremely close given different noise on prior.
