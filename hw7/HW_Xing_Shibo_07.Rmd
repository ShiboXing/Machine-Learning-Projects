---
title: "CS 1675 Spring 2020: Homework 07"
...
prob_01_df <- readr::read_csv("https://raw.githubusercontent.com/jyurko/CS_1675_Spring_2020/master/hw_data/hw07/hw_07_prob_01_data.csv", col_names = TRUE)
...
```
I think the continuous inputs are all minutely correlated to each other as their correlation plots all have very small but non-zero
values.
...
```{r, solution_02b}
set.seed(3321)
fit_01_lm <- train(y ~ ., data=prob_01_df, method='lm', preProcess=c('center', 'scale'), metric=metric_01, trControl=ctrl_k05)

fit_01_lm
```
70 observatins are used to test each fold and the cross-validation averaged R-squared is 0.4391936
...
```
Yes, x4 and x5 coefficients interval contain 0. That means those coefficients might be inessential to the response.
...
Since the input X1 has 3 possible values A, B and C, and X2 has only 2, we have to derive two variables for X1 but only one for X2 for the sake of their discrete values. In total, we have 8 coefficients (including the response).
...
```{r, solution_02f}
set.seed(3321)
fit_01_lm_pairs <- train(y ~ (.)^2, data=prob_01_df, method='lm', preProcess=c('center', 'scale'), metric=metric_01, trControl=ctrl_k05)
...
```
the interation terms x1B:x2b and x1C:x2b seems to indicate a strong correlation of their constituent variables.
...
```{r, solution_02h}
set.seed(3321)
fit_01_glmnet_trips <- train(y ~ (.)^3, data=prob_01_df, method='glmnet', preProcess=c('center', 'scale'), metric=metric_01, trControl=ctrl_k05)

fit_01_glmnet_trips
```
Since RMSE is used to select the optimal model, the optimal model has alpha = 0.55 and lambda = 0.3509.
Since alpha of this elastic net is 0.55, we could say that the trained elastic net mode is slightly more like LASSO.
...
fit_01_rf
```
Preprocessing doesn't matter since the scale or how normalized the observations are doesn't matter since decision tree-based methods only concern with classification.
the mtry value 7 is considered the best. It indicates the number of variables available for splitting at each tree node.
...
I think yes. Like why we did preprocess for our previous dataset, the 0-1 range is actually pretty large compared to the individual ranges of the inputs. The middle 90 percentiles of many inputs are completely separate from each other. 
...
```{r, solution_5b}
set.seed(4321)
fit_glm_sonar <- train(Class ~ ., data = Sonar, method = 'glm', metric = metric_sonar, trControl = ctrl_k05_roc, preProc = c('scale', 'center'))
...
```{r, solution_05c}
set.seed(4321)
fit_glmnet_sonar <- train(Class ~ ., data = Sonar, method = 'glmnet', preProcess=c('center', 'scale'), metric=metric_sonar, trControl=ctrl_k05_roc)
...
```{r, solution_05d}
set.seed(4321)
fit_nnet_sonar <- train(Class ~ ., data = Sonar, method = 'nnet', preProcess=c('center', 'scale'), metric=metric_sonar, trControl=ctrl_k05_roc, trace = FALSE)
...
```{r, solution_05e}
set.seed(4321) 
fit_rf_sonar <- train(Class ~ ., data = Sonar, method = 'rf', preProcess=c('center', 'scale'), metric=metric_sonar, trControl=ctrl_k05_roc, importance = TRUE)
...
```{r, solution_05f}
set.seed(4321)
fit_xgb_sonar <- train(Class ~ ., data = Sonar, method = 'xgbTree', preProcess=c('center', 'scale'), metric=metric_sonar, trControl=ctrl_k05_roc, importance = TRUE)
```
