...
```
I think the counts x2's As and Bs when x1 is C dominate the data set. 
...
fit_01_lm
```
70 observatins are used to test each fold and the cross-validation averaged R-squared is 0.4079824
...
```
Yes, x4 and x5 coefficients interval contain 0.
...
```{r, solution_03d_a}
reg_mod_compare <- resamples(list(LM = ,
                                  LMpairs = ,
                                  GLMNET = ,
                                  NNET = ,
                                  RF = ,
...
```{r, solution_05g}
sonar_roc_compare <- resamples(list(GLM = ,
                                    GLMNET = ,
                                    NNET = ,
                                    RF = ,
                                    XGB = ))
...
```{r, solution_06d}
pred_test_prob_sonar_glmnet <- 

pred_test_prob_sonar_rf <- 
```
