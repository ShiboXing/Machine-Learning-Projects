...
post_samples_01 %>% 
  ggplot() +
  geom_histogram(mapping = aes(x = beta_01))
```
...
filter(all_spline_mae_results, dataset == 'training')$mae %>% summary
filter(all_spline_mae_results, dataset == 'test')$mae %>% summary
```
...
p(\boldsymbol{\beta}) = \prod_{d=1}^{D}(normal(\boldsymbol{\beta_d}|\boldsymbol(\mu_\beta, \tau_\beta)))\\
...
Since generate_lm_post_samples takes in a mvn_result where the mode's parameters contain also the standard deviation.
...
post_pred_summary_03_line_b <- 

post_pred_summary_03_cube_b <- 
```
...
```{r, merge_preds_and_train_obs_03, eval=FALSE}
