...
Ziyi Gong, Siyu Zhang, Yuanpu Ma
...
#### SOLUTION
the relationship between the response and input is linear relationship
...
Xmat_01 <- model.matrix(y ~ x, train_01 %>% data.frame())
...
```{r, solution_01d_c}
laplace_01$mode[1:2] 
laplace_01$mode[3] %>% exp()
```

No we should not have that convern since we know that the posterior surface is unimodal and gradient descent with any initial guesses will give us the unique mode parameters.
...
set.seed(87123)
post_samples_01 <- generate_lm_post_samples(laplace_01, 2, 2500) 
...
post_samples_01 %>% 
  ggplot() +
  geom_histogram(bins = 55, mapping = aes(x = beta_01))
```

The posterior probability that the slope is greater than zero is:  

```{r, solution_01e_d, eval=TRUE}
sum(post_samples_01$beta_01 > 0) / nrow(post_samples_01)
```
...
error_01_post_01 <- post_pred_samples_01$Umat[,1] %>% as.vector - train_01$y

### error of the second posterior sample
error_01_post_02 <- post_pred_samples_01$Umat[,2] %>% as.vector - train_01$y

### error of the third posterior sample
error_01_post_03 <- post_pred_samples_01$Umat[,3] %>% as.vector - train_01$y
```

Since our model is a first-order linear model, we have a prediction spline where the prediction interval is significantly wider than the confidence interval. Thus, we can calculate the error through mean trend to get a more accurate error.
...
I am averaging over the absolute errors of mean trend samples produced by the 50 inputs and the first Beta sample, whereby the absolute error the the difference between the mean trend and the response (observation).
...
mae_01_post_01 <- abs(error_01_post_01) %>% mean

mae_01_post_02 <- abs(error_01_post_02) %>% mean

mae_01_post_03 <- abs(error_01_post_03) %>% mean
```

they are different since we are calculate the error of the means trends based on three different samples of beta matrix, which means that the mean trends values would be different, which in turn makes the errors different. 
...
It is the colMeans function I should use, since each column of each Umat matrix is a posterior result based on a sample of the Beta matrix

```{r, solution_02f, eval=TRUE}
MAE_01 <- absE01mat %>% colMeans
...
MAE_01[1:3]
c(mae_01_post_01, mae_01_post_02, mae_01_post_03)

MAE_01[1:3] == c(mae_01_post_01, mae_01_post_02, mae_01_post_03)
```
...
Since we drew the beta matrix samples from the mvn distribution we approximated using laplace, it is natural that we get a distribution of MAE when using these Beta samples to calcula the mean trend. The cause of the MAE uncertainty is the draws from the distribution of Beta matrices. 
...
ggplot() +
  geom_point(mapping = aes(x = train_02$x, y = train_02$y), color='red') +
  geom_point(mapping = aes(x = test_02$x, y = test_02$y), color='blue')
```
...
There are 150 rows in the training matrix and 50 rows in testing matrix
...
calc_mae_from_laplace <- function(mvn_result, Xtrain, Xtest, y_train, y_test, num_samples)
{
  ...
  J = mvn_result$mode %>% length - 1
  post <- generate_lm_post_samples(mvn_result, J, num_samples)
  ...
  mae_train <- error_train %>% abs %>% colMeans
  
  # calculate the MAE on the test set
  mae_test <- error_test %>% abs %>% colMeans
...
all_spline_mae_results %>% filter(J > 3) %>% ggplot(mapping = aes(x = J, y = mae)) +
                                                    stat_summary(geom = 'line', fun.y = 'median') +
                                                    facet_wrap(facets = ~ dataset)

```

splines of order 6 or higher seem to overfit the observations. 
...
spline_weights <- spline_evidence %>% exp / sum(spline_evidence %>% exp)
...
the fifth-order model is the best
...
They not too different. Evidence measures how much the posterior is consistent with priors. Data split measures how well the model generalizes. 
...
$$
\begin{aligned}
\boldsymbol{\eta_n} = \mathbf{x}_{n,:} \boldsymbol{\beta} \\
p(\boldsymbol{\beta}) = \prod_{d=1}^{D+1}(normal(\boldsymbol{\beta_d}|\boldsymbol(\mu_\beta, \tau_\beta)))\\
y|\beta_0, \beta_1, x \propto \prod_{n=1}^{N}(Bernoulli(y_n|logit^{-1}[\boldsymbol{\eta_n}]) p(\boldsymbol{\beta}) 
\end{aligned}
$$
...
Xmat_03_line <- model.matrix(y ~ x, train_03 %>% data.frame)

Xmat_03_cube <- model.matrix(y ~ (I(x) + I(x^2) + I(x^3)), train_03 %>% data.frame)
```
...
No I do not need to separate out the beta parameters because the priors of mu and tau are already specified my_info. unknowns vector only consists of the beta coefficients
...
glm_logpost <- function(unknowns, my_info)
{
  ...
  # evaluate the log-likelihood
  log_lik <- sum(dbinom(x = my_info$y,
                        size = 1,
                        prob = mu,
                        log = TRUE))
...
No I should not because the log posterior is still unimodal one.
...
sigma = laplace_03_line$var_matrix %>% diag %>% sqrt
pnorm(0.975, laplace_03_line$mode, sigma)
pnorm(0.025, laplace_03_line$mode, sigma)

sigma = laplace_03_cube$var_matrix %>% diag %>% sqrt
pnorm(0.975, laplace_03_cube$mode, sigma)
pnorm(0.025, laplace_03_cube$mode, sigma)
# laplace_03_line$mode %>% quantile(seq(0.025, 0.975, 0.9))
# laplace_03_cube$mode %>% quantile(seq(0.025, 0.975, 0.9))
```
It seems like none of the parameters' 95% uncertainty intervals contain 0.
...
laplace_03_line$log_evidence %>% exp / laplace_03_cube$log_evidence %>% exp
```

It seems like the linear model supported more by the data.  
...
laplace_03_cube$var_matrix %>% cov2cor
```
...
Because generate_lm_post_samples takes in a mvn_result where the mode's parameters contain also the standard deviation, our mvn_result's mode only consists of beta parameters.
...
summarize_glm_pred_from_laplace <- function(mvn_result, Xtest, num_samples)
{
  ...
  betas <- MASS::mvrnorm(num_samples, mvn_result$mode, mvn_result$var_matrix)
...
post_pred_summary_03_line <- summarize_glm_pred_from_laplace(laplace_03_line, info_03_line$design_matrix, 2500)

post_pred_summary_03_cube <- summarize_glm_pred_from_laplace(laplace_03_cube, info_03_cube$design_matrix, 2500)
...
post_pred_summary_03_line %>% dim
post_pred_summary_03_cube %>% dim
```
...
The linear relationship is more in-line with the observations
...
labels = c('true negative', 'false negative', 'false positive', 'true positive')
mt_line = post_pred_summary_03_line_c %>% count(pred_y, y)
mt_line %>% mutate('label' = labels)
```
...
labels = c('true negative', 'false negative', 'false positive', 'true positive')
mt_cube = post_pred_summary_03_cube_c %>% count(pred_y, y)
mt_cube %>% mutate('label' = labels)
```
